...
class DownloadThread implements Runnable {
    ...
    private void runInternal() {
        ...
        if (downloadStatus == DownloadsStatus.STATUS_SUCCESS) {
            Log.d("Download " + originalDownloadInfo.getId() + " already finished; skipping");
            return;
        }
        if (DownloadsStatus.isStatusCancelled(downloadStatus)) {
            Log.d("Download " + originalDownloadInfo.getId() + " already cancelled; skipping");
            return;
        }
        if (DownloadsStatus.isStatusError(downloadStatus)) {
...
//            TrafficStats.setThreadStatsUid(downloadInfo.uid); Won't need this as we will be an Android library (doing own work)
            ...
            try {
                ...
                throw new StopRequestException(DownloadsStatus.STATUS_BAD_REQUEST, e);
            }

            executeDownload(state);

            finalizeDestinationFile(state);
            finalStatus = DownloadsStatus.STATUS_SUCCESS;
            ...
            if (finalStatus == DownloadsStatus.STATUS_WAITING_TO_RETRY) {
            ...
            if (isStatusRetryable(finalStatus)) {
                ...
                if (numFailed < Constants.MAX_RETRIES) {
                    ...
                    if (info != null && info.getType() == state.networkType && info.isConnected()) {
                        ...
                        finalStatus = DownloadsStatus.STATUS_WAITING_TO_RETRY;
                    } else {
                        // Network changed, retry on any next available
                        finalStatus = DownloadsStatus.STATUS_WAITING_FOR_NETWORK;
                    }
                }
            }
            ...
            Log.w(msg, ex);
            finalStatus = DownloadsStatus.STATUS_UNKNOWN_ERROR;
...
//            TrafficStats.clearThreadStatsUid();
            ...
            Log.i("Download " + originalDownloadInfo.getId() + " finished with status " + DownloadsStatus.statusToString(finalStatus));
...
//            netPolicy.unregisterListener(mPolicyListener);
    ...
    private void executeDownload(State state) throws StopRequestException {
                ...
                switch (responseCode) {
                    case HTTP_OK:
                        if (state.continuingDownload) {
                            throw new StopRequestException(DownloadsStatus.STATUS_CANNOT_RESUME, "Expected partial, but received OK");
                        }
                    ...
                    case HTTP_PARTIAL:
                        if (!state.continuingDownload) {
                            throw new StopRequestException(DownloadsStatus.STATUS_CANNOT_RESUME, "Expected OK, but received partial");
                        }
                        transferData(state, conn);
                        return;
                    ...
                    case HTTP_REQUESTED_RANGE_NOT_SATISFIABLE:
                        throw new StopRequestException(DownloadsStatus.STATUS_CANNOT_RESUME, "Requested range not satisfiable");
        ...
        throw new StopRequestException(DownloadsStatus.STATUS_TOO_MANY_REDIRECTS, "Too many redirects");
    }
...
//                    outFd = file.getFD();
                ...
                throw new StopRequestException(DownloadsStatus.STATUS_FILE_ERROR, e);
            }
...
//            }
    ...
    private void checkConnectivity() throws StopRequestException {
        ...
        if (networkUsable != NetworkState.OK) {
            int status = DownloadsStatus.STATUS_WAITING_FOR_NETWORK;
...
//            Log.v("downloaded " + state.currentBytes + " for " + downloadInfo.uri);
    ...
    private void cleanupDestination(State state, int finalStatus) {
        if (state.filename != null && DownloadsStatus.isStatusError(finalStatus)) {
        ...
        while (true) {
            try {
                ...
                if (!forceVerified) {
                    ...
                    throw new StopRequestException(DownloadsStatus.STATUS_FILE_ERROR,
...
//                throw new StopRequestException(Downloads.Impl.STATUS_FILE_ERROR, "found invalid internal destination filename");
        ...
        if (destinationFile.exists()) {
            ...
            if (fileLength == 0) {
                ...
                destinationFile.delete();
                throw new StopRequestException(DownloadsStatus.STATUS_CANNOT_RESUME, "Trying to resume a download that can't be resumed");
            } else {
     ...
     * {@link DownloadsStatus#STATUS_WAITING_TO_RETRY}.
     */
